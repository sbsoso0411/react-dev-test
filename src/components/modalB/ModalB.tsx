...
export default function ModalB(props: ModalBProps) {
  ...
  return <>
    <Modal size="lg" centered backdrop="static" show={show} animation={false}>
      <Modal.Header style={{ 'backgroundColor': '#ff7f50', 'color': 'white' }}>
        ...
        <Scrollbars onUpdate={onScroll} style={{ width: '100%', height: '300px' }}>
          <Table striped bordered hover>
            ...
            <tbody>
              ...
              </> : (onlyEven ? data.contact_ids.filter((contact_id: number) => (contact_id % 2 === 0 && data.contacts[contact_id].country_id === 226)) : data.contact_ids.filter((contact_id: number) => data.contacts[contact_id].country_id === 226)).map((contact_id: number) => {
